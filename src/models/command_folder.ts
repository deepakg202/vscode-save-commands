...
import { JSONObj } from "./base_types";
...
export class CommandFolder {
  ...
  static create(fields: { name: string, parentPath?: string, sortOrder?: number }) {
    return new CommandFolder({ id: uuidv4(), name: fields.name, parentPath: fields.parentPath, sortOrder: fields.sortOrder });
  }
  ...
  static getWorkspaceFolders(context: ExtensionContext): Array<CommandFolder> {
    const commands = (context.workspaceState.get<Array<JSONObj>>(COMMAND_FOLDERS_STORAGE_KEY) ??
      []);
  ...
  static getGlobalFolders(context: ExtensionContext): Array<CommandFolder> {
    const commands = (context.globalState.get<Array<JSONObj>>(COMMAND_FOLDERS_STORAGE_KEY) ??
      []);
    return commands.map((value) => CommandFolder.fromJson(value));
  }
}
